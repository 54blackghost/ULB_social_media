...
const ChatBox = () => {
  ...
  return user && (
    <div className='flex flex-col h-screen'>
       ...
       <div className='p-5 md:px-10 h-full overflow-y-scrool'>
          <div className="space-y-4 max-w-4xl mx-auto">
            {
              messages.toSorted((a,b)=> new Date(a.createdAt) - new Date(b.createdAt)).map((message, index)=>(
                <div key={index} className={`flex flex-col ${message.to_user_id !== 
                  user._id ? 'items-strat' : 'items-end'
                }`}>
       ...
       <div className="px-4">
        <div className="flex imtes-center gap-3 pl-5 p-1.5 bg-white w-full 
         max-w-xl mx-auto border border-gray-200 shadow rounded-full mb-5">
          ...
          <label htmlFor="image">
            {
              image 
              ? <img src={URL.createObjectURL(image)} alt="" srcset="" className='h-8 rounded' />
          ...
          <button onClick={sendMessage} className='bg-gradient-to-r from-indigo-500 to-purple-600 hover:from-indigo-700
